...
const versions = ['NIL', 'v1', 'v3', 'v4', 'v5', 'v7', 'v8'] as const;
...
const validUuidRules = [
  {
    ...
    validator: (value: string) => {
      ...
      // Updated regex to support UUID v1-v8
      return Boolean(value.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/));
    },
  },
];

// UUID v8 generator - custom implementation with random data
function generateUuidV8(): string {
  const randomBytes = new Uint8Array(16);
  crypto.getRandomValues(randomBytes);

  // Set version to 8 (0b1000) in the 7th byte (high nibble)
  randomBytes[6] = (randomBytes[6] & 0x0F) | 0x80;

  // Set variant to RFC 4122 (0b10xx) in the 9th byte
